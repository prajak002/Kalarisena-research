...
Trained on the six-motion stable-stance family, `com_refine_env.py`. Two real observation bugs were found and fixed here: a duplicated `cp_margin` value occupying the slot meant to encode support mode, and an unguarded `-999.0` sentinel (`PinocchioWrapper.get_support_features`'s placeholder for "no foot contact detected") reaching the policy's input uncapped whenever contact was briefly lost — exactly the moment a clean balance signal matters most. Both fixed; the retrained checkpoint's fall rate is unchanged: **100%**, mean capture-point margin **−0.223** (negative meaning the capture point sits outside the support polygon on average). A subsequent attempt warm-starting Stage B directly from Stage A's tracking weights (`src/rl/warm_start.py`, matching the design document's Table 34 specification, which the original from-scratch training did not follow) is in progress at the time of this draft; see Section 9.
...
Under honest measurement, this project has not yet reduced Kalaripayattu fall rate below what a policy taking no action at all achieves, across six independently tried approaches (the original tracker, an expanded corpus, a curriculum-pretrained variant, Stage B's dedicated balance objective, static-stance pretraining, and the warm-start attempt in progress). It has also not yet produced genuine standing recovery after a fall, at 0% under a corrected, meaningful success criterion after 20 million further training steps. These are real, unresolved negative results about the difficulty of dynamic martial-arts movement on this hardware, reported as such.

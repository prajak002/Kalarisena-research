# KalariSena: Physics-Grounded, Fall-Safe Kalaripayattu Skill Transfer for Humanoid Robots
...
Humanoid control for highly dynamic whole-body motion remains brittle because accurate tracking alone does not ensure stability, recoverability, or safe failure. This challenge is especially acute in Kalaripayattu, where a robot must execute authentic motions involving deep stances, one-leg support, rapid center-of-mass shifts, kicks, pivots, and abrupt support transitions that push it toward the limits of balance and contact feasibility. Since falls under this regime turn out to be effectively unavoidable with the control stack tested here, we ask a narrower, practically important question: when a fall happens, can a trained policy make it measurably safer than doing nothing? We built a unified curriculum for the Unitree G1 in MuJoCo/Pinocchio spanning six stages — reference tracking, CoM/capture-point recoverability, momentum regulation, fall-impact minimization, recovery-to-standing, and threshold-based mode switching — and evaluated it across a 139-motion Kalaripayattu corpus with genuine held-out generalization splits.

The central positive result of this paper: a fall-impact-minimization policy, evaluated under a corrected protocol that measures force through actual ground contact rather than at the moment a fall is first detected, reduces peak impact force by 38% on average and up to 58% in the worst case relative to a no-action baseline, uniformly across all three motion families tested (54% reduction on stable-stance falls, 49% on translational falls, 18% on the hardest explosive-strike falls). This is a genuine, reproducible safety improvement obtainable without solving balance or recovery at all — a robot that is going to fall anyway can still be trained to fall less violently. The remaining results are honestly negative and we report them as such: every tracking-stage variant we tried — the original twelve-motion tracker, a 125-motion expanded corpus, a curriculum-pretrained variant, and Stage B's dedicated balance objective, from scratch and warm-started — lands at 99–100% fall rate, statistically indistinguishable from a policy that takes no action at all, and the recovery-to-standing policy remains at 0% genuine success even after twenty million steps against a corrected success criterion. We separately report three systemic evaluation bugs found and fixed during this work, each of which had been making a reported result look substantially better than the underlying policy's real behavior — one by up to two orders of magnitude, including the original (erroneous) version of the impact-reduction number itself — and we treat correcting them, plus the fact that the impact-reduction finding survived correction while the others did not, as this paper's central, reproducible contribution.
...
**The positive result this paper stands on.** Balance and recovery did not work in this project, but fall-impact minimization did, and it is the more practically load-bearing of the two for a robot that is actually going to be deployed: any legged system doing dynamic, high-amplitude motion will eventually fall, whether or not its balance controller is good, simply from disturbances, contact surprises, or motions at the edge of feasibility. A policy that reduces peak impact force by roughly 38% on average, and by more than half on the stable-stance and translational fall families specifically, lowers the chance of hardware damage or injury on every one of those falls — independent of whether the robot ever learns to avoid falling in the first place, and independent of whether it can later stand back up. This result is real: it is measured through actual ground contact rather than at first fall-detection (the original, erroneous version of this same measurement — using episode termination as the stopping point — is documented in Section 8.4 as one of the three evaluation bugs this paper corrects), it holds uniformly across all three motion families tested rather than only in a favorable subset, and it is reproducible from `logs/stageD_fall_corrected/` with the exact git commit and simulator version recorded in `meta.json`.
...
**The methodological contribution.** Three separate, systemic evaluation bugs were found and fixed during this work, each of which had made a previously reported result look substantially better than the policy's actual behavior — a fall-rate evaluation bug that inflated an internally-reported 83% survival rate to a false positive (honest number: ~100% fall rate), an early-termination bug that understated Stage D's real peak impact force by roughly two orders of magnitude (this is the same bug whose correction produced the paper's central positive result above — fixing it changed the number, not the direction of the finding), and a height-blind success criterion that reported 15% genuine standing recovery where the real rate is 0%. In every case, the corrected number was found and reported before it was used elsewhere, not after. We regard the discipline of re-measuring past the point where an earlier evaluation happened to stop — rather than trusting that stopping point — as directly applicable to evaluation of any RL-trained recovery or fall-mitigation behavior, independent of this specific robot, motion corpus, or martial art, and as the reason the one positive result in this paper can be trusted: it survived exactly the kind of re-measurement that reversed or zeroed out every other headline number here.
